...
import { BiComment, BiHeart, BiLike, BiSolidHeart, BiSolidLike } from "react-icons/bi";
...
const PostCard = ({ post, user, deletePost, likePost }) => {
  ...
  const getPostAiResponse = async (id) => {
    try {
      const res = await apiRequest({
        url: "/posts/ai-response/" + id,
        methods: "GET"
      });
  
      console.log("Fetching AI response for post:", id);
  
 
      setAiResponse(res[0]?.comment);
  ...
  return (
    <div className='mb-2 bg-[#121230] p-4 rounded-xl'>
      ...
      >
        ...
        <p className='flex gap-2 items-center text-base cursor-pointer' onClick={() => {
          ...
          {post?.likes?.includes(user?._id) ? (
            <BiSolidHeart size={20} color='#3ed0e7' />
          ) : (
            <BiHeart size={20} />
          )}
        ...
        >
          <BiComment size={20} />
          {post?.comments?.length} Commentários
        </p>

        {user?._id === post?.userId?._id && (
          <div
            className='flex gap-1 items-center text-base text-ascent-1 cursor-pointer'
            onClick={() => deletePost({id: post?._id, token: user.token})}
          >
